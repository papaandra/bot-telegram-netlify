...
// Mengimpor library node-fetch untuk melakukan permintaan HTTP (pengganti fetch bawaan Deno).
// Karena node-fetch tidak ada di environment Netlify secara default, kita akan install.
const fetch = require('node-fetch');

// Mengimpor library telegraf untuk interaksi mudah dengan Telegram Bot API (alternatif TeleBot).
// Ini juga akan diinstal melalui package.json.
...
if (!TELEGRAM_BOT_TOKEN || !NETLIFY_ACCESS_TOKEN) {
  ...
  // Di Node.js, kita kembalikan respons error.
  return {
    statusCode: 500,
...
bot.on('callback_query', async (ctx) => {
  ...
  await ctx.answerCbQuery(); // Menjawab callback query (penting agar tidak ada loading di tombol)
